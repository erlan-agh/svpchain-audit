...
// Lifecycle of a compliance-gated deposit. Funds are custodied by the
// contract from Pending until either Approved (Deposit emitted, bridging
// proceeds) or refunded back to the depositor (Rejected → Refunded, or a
// timed-out Pending → Refunded). Once Approved, the order can never be
// refunded; once Rejected/Refunded it can never be approved — this single
// mapping is the double-spend guard for "bridged AND refunded".
enum OrderStatus { None, Pending, Approved, Rejected, Refunded }

// One per deposit(). `user` is the source-chain depositor (refunds go here,
// NOT `destination`, which is the recipient on the target chain).
// `token == NATIVE_TOKEN` means the locked asset is native.
struct DepositOrder {
  address user;
  address token;
  uint256 amount;
...
contract SVPBridge is Pausable, ReentrancyGuard {
  ...
  // ─── Compliance gating (KYC/KYA) ───────────────────────────────────────────
  // deposit()/depositNative() no longer emit Deposit directly; they custody the
  // funds and emit PreDeposit against a fresh orderId. A kyacOperator later calls
  // processKYAC(orderId, approved): approved=true emits the real Deposit (bridging
  // proceeds), approved=false marks the order Rejected so the depositor can refund.
  // Because compliance status for an address can change over time, the verdict is
  // stored per-order rather than per-address.
  mapping(uint256 => DepositOrder) public depositOrders;
  uint256 public nextOrderId; // ++nextOrderId, so live orderIds start at 1 (0 = invalid)
  mapping(address => bool) public kyacOperators; // cold-key governed compliance signers
  uint64 public kyacTimeoutSeconds; // Pending longer than this → depositor may self-refund
  ...
  // PreDeposit is emitted by deposit()/depositNative() the moment funds are
  // custodied. The backend reads it, runs KYC/KYA on `user`, then calls
  // processKYAC(orderId, ...). orderId ties the whole lifecycle together
  // (PreDeposit → Deposit / Refunded) and is echoed on the Deposit event below.
  event PreDeposit(
  ...
  // ─── Events: Compliance (KYC/KYA) ──────────────────────────────────────────
  event KYACProcessed(uint256 indexed orderId, bool approved);
  event Refunded(uint256 indexed orderId, address indexed user, address token, uint256 amount);
  ...
  event ModifiedKyacOperator(address indexed operator, bool isOperator);
  event ChangedKyacTimeoutSeconds(uint64 newKyacTimeoutSeconds);
  ...
  constructor(
    ...
    address[] memory _kyacOperators,
    uint64 _kyacTimeoutSeconds
  ) {
    ...
    kyacTimeoutSeconds = _kyacTimeoutSeconds;
    // Seed compliance signers at deploy time (same moment as hot/cold).
    // Later grants/revokes still go through cold-key modifyKyacOperator.
    for (uint256 i = 0; i < _kyacOperators.length; i++) {
      address op = _kyacOperators[i];
      require(op != address(0), "Invalid kyac operator");
      kyacOperators[op] = true;
      emit ModifiedKyacOperator(op, true);
    }
  ...
  function deposit(
    ...
    _openDepositOrder(token, amount, targetChainId, targetToken, destination);
  }
  ...
  function depositNative(
    ...
    _openDepositOrder(NATIVE_TOKEN, msg.value, targetChainId, targetToken, destination);
  }

  // Custody the (already-locked) funds under a fresh order in Pending and emit
  // PreDeposit. The real Deposit event — which the backend bridges on — is only
  // emitted later by processKYAC once compliance passes.
  function _openDepositOrder(
    address token,
    uint256 amount,
    uint64 targetChainId,
    address targetToken,
    ...
    depositOrders[orderId] = DepositOrder({
      user: msg.sender,
      token: token,
      amount: amount,
    ...
    emit PreDeposit(orderId, msg.sender, token, amount, targetChainId, targetToken, destination);
  }

  // ─── Compliance: KYAC & Refund ─────────────────────────────────────────────

  // Record the compliance verdict for a Pending order. Only a kyacOperator may
  // call. approved=true emits the real Deposit event (the backend bridges on
  // this, exactly as before) and moves the order to Approved. approved=false
  // moves it to Rejected so the depositor can pull a refund. The Pending-only
  // guard makes every transition one-way and idempotent — an order can never be
  // both bridged and refunded. No external calls here, so no reentrancy surface.
  function processKYAC(uint256 orderId, bool approved) public whenNotPaused {
    require(kyacOperators[msg.sender], "Sender is not a kyac operator");
    DepositOrder storage o = depositOrders[orderId];
    require(o.status == OrderStatus.Pending, "Order not pending");
    if (approved) {
      o.status = OrderStatus.Approved;
      emit Deposit(o.user, o.token, o.amount, o.targetChainId, o.targetToken, o.destination, orderId);
    } else {
      o.status = OrderStatus.Rejected;
    }
    emit KYACProcessed(orderId, approved);
  }

  // Batch verdicts in one tx (the backend leader submits many at once, mirroring
  // batchedFinalizeWithdrawals). Per-order authorization / state checks live in
  // processKYAC.
  function batchedProcessKYAC(
    uint256[] calldata orderIds, bool[] calldata approvals
  ) external {
    require(orderIds.length == approvals.length, "Length mismatch");
    uint64 end = uint64(orderIds.length);
    for (uint64 idx; idx < end; idx++) {
      processKYAC(orderIds[idx], approvals[idx]);
    }
  }

  // Depositor pulls their funds back when either:
  //   • the order was Rejected by KYAC, or
  //   • the order is still Pending past kyacTimeoutSeconds (backend never
  //     ruled — protects the depositor from indefinite custody).
  // Pull model + checks-effects-interactions + nonReentrant: status flips to
  // Refunded before the transfer, so a reentrant call re-hits the state guard.
  // Funds return to the source-chain depositor (o.user), never to destination.
  function refund(uint256 orderId) external nonReentrant whenNotPaused {
    DepositOrder storage o = depositOrders[orderId];
    require(o.user == msg.sender, "Not order owner");
    bool rejected = o.status == OrderStatus.Rejected;
    bool timedOut = o.status == OrderStatus.Pending
      && block.timestamp > o.createdAt + kyacTimeoutSeconds;
    require(rejected || timedOut, "Order not refundable");
    o.status = OrderStatus.Refunded;
    _transferOut(o.token, o.user, o.amount);
    emit Refunded(orderId, o.user, o.token, o.amount);
  }
  ...
  // Grant / revoke the KYAC compliance role. Cold-key governed: authorizing an
  // address to approve or reject deposits is as sensitive as token support, so
  // both grant and revoke require a cold quorum (unlike modifyFinalizer, whose
  // grant path uses the hot set).
  function modifyKyacOperator(
    address operator, bool _isOperator, uint64 nonce,
    ValidatorSet memory coldVS, Signature[] memory sigs
  ) external {
    bytes32 data = keccak256(abi.encode("modifyKyacOperator", operator, _isOperator, nonce));
    bytes32 message = _makeMessage(data);
    _checkMessageNotUsed(message);
    _checkValidatorSignatures(message, coldVS, sigs, coldValidatorSetHash);
    kyacOperators[operator] = _isOperator;
    emit ModifiedKyacOperator(operator, _isOperator);
  }

  function changeKyacTimeoutSeconds(
    uint64 val, uint64 nonce, ValidatorSet memory coldVS, Signature[] memory sigs
  ) external {
    bytes32 data = keccak256(abi.encode("changeKyacTimeoutSeconds", val, nonce));
    bytes32 message = _makeMessage(data);
    _checkMessageNotUsed(message);
    _checkValidatorSignatures(message, coldVS, sigs, coldValidatorSetHash);
    kyacTimeoutSeconds = val;
    emit ChangedKyacTimeoutSeconds(val);
  }
